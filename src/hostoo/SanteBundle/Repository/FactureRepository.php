...
class FactureRepository extends EntityRepository
{
    ...
    public function recetteDuMoisPatient($annee)
    {
        $qb=$this->createQueryBuilder('a');
        $qb->select('acte.designationacte as ac, COUNT(tar) as nbr, SUM(tar.prix) as montant')
            ->join('a.commandes','cmd')
            ->join('cmd.tarifs','tar')
            ->join('tar.actes','acte')
            ->where('a.datefacture LIKE :mois')
            ->setParameter('mois',$annee.'%')
            ->groupBy('acte');
        return $qb->getQuery()->getResult();

    }

    public function recetteParActe()
    {
        $qb=$this->createQueryBuilder('a');
        $qb->select('a')
            ->join('a.episode','e')
